...
mention_categories = ['PER', 'LOC', 'ORG', 'PKM', 'OBJ', 'MISC']
...
class MentionFeatures:
    ...
    def add(self, trees, embeddings, mentions):
        ...
        for mention in mentions:
            # feature indicators:
            # ['nh', 'h', 'f', 'm', 'n', 'sg', 'pl', 'PER', 'LOC', 'ORG', 'PKM', 'OBJ', 'MISC']
            # multiple values can be True!
            # if a feature is unknown, all of its possible values will be False

            # Use the head category only
            try:
                mention.features['category'] = mention.features['category'].split('.')[0]
            except KeyError:
                mention.features['category'] = None
            ...
            for count, category in enumerate(mention_categories):
                label[count + 7] = mention.features['category'] == category
            ...
            result.append((
                ...
                # Alpino named entities
                mention.head.get('neclass') == 'PER',
                mention.head.get('neclass') == 'LOC',
                mention.head.get('neclass') == 'ORG',
                mention.head.get('neclass') == 'MISC',
            ))
...
def evaluate(validationfiles, parsesdir, annotations, tokenizer, bertmodel):
    ...
    target_names = ['nonhuman', 'human', 'female', 'male', 'neuter',
                    'singular', 'plural'] + mention_categories

    print('\nperformance of feature classifier:')
    print(classification_report(
        np.array(y_val[:, -6:], dtype=bool),
        np.array([a > 0.5 for a in probs[:, -6:]], dtype=bool),
        target_names=mention_categories,
        zero_division=0,
        digits=3))


def predict(trees, embeddings, mentions, verbose=False, numthreads=1):
    """Load mentions classifier, get features for mentions,
    and update features of mentions."""
    ...
    for row, mention in zip(probs, mentions):
        ...
        elif row[5] < 0.5 < row[6]:
            mention.features['number'] = 'pl'
        else:
            mention.features['number'] = None
        category_probabilities = row[7:]
        mention.features['category'] = mention_categories[np.argmax(category_probabilities)]
        if verbose:
...
def main():
    ...
    if opts.get('--import'):
        ...
        annotations = pd.concat(result).set_index(
            ...
            ['gender', 'number', 'category']].T.to_dict()
    if opts.get('--eval'):
